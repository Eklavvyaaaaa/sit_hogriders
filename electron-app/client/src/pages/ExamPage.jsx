...
const ExamPage = () => {
    ...
    useEffect(() => {
        ...
        navigator.mediaDevices.getUserMedia({ video: true, audio: false }) // Audio disabled to prevent feedback loop by default, can be enabled if needed
            .then(stream => {
        ...
        socket.on('webrtc:request', async ({ teacherId, examId: reqExamId }) => {
            if (String(reqExamId) !== String(examData.examId)) return;

            // Create new peer connection for this requesting teacher
            ...
            // Create Offer and send to Teacher
            try {
        ...
        socket.on('webrtc:answer', async ({ teacherId, answer, examId: reqExamId }) => {
            if (String(reqExamId) !== String(examData.examId)) return;
            const pc = peers[teacherId];
            if (pc) {
                try {
        ...
        socket.on('webrtc:ice-candidate', async ({ senderId: teacherId, candidate, examId: reqExamId }) => {
            if (String(reqExamId) !== String(examData.examId)) return;
            const pc = peers[teacherId];
